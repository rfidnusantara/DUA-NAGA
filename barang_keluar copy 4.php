...
// ==========================
// 1c. Ambil data API → Mapping PO -> raw name/address (bisa berisi (1)...(2)...)
// ==========================

// fetch_sales_orders_from_api bisa mengembalikan array langsung atau ['data'=>[]]
$rawApi = fetch_sales_orders_from_api(1, 50);

if (is_array($rawApi) && isset($rawApi['data']) && is_array($rawApi['data'])) {
    $salesOrdersApi = $rawApi['data'];
} else {
    $salesOrdersApi = $rawApi;
}

/**
 * $poCustomerMap = [
 *   'PO-001' => [
 *       'name_raw'    => '(1) LUKI ...(2) BAGASKORO ...',
 *       'address_raw' => '(1) Jl....(2) Perum ...',
 *   ],
 *   ...
 * ];
 */
...
if (is_array($salesOrdersApi)) {
    foreach ($salesOrdersApi as $order) {
        ...
        // Kalau ada beberapa order dengan PO sama, kita timpa saja (isinya sama)
        $poCustomerMap[$po] = [
                    ...
                    <div class="mb-4">
                        ...
                        <div class="card bg-light border-0 p-3">
                            <div class="mb-2">
                                ...
                                <!-- Dropdown pilihan customer (kalau >1) -->
                                <div id="customer_choice_wrapper" class="mt-2" style="display:none;">
                                    <select id="customer_choice" class="form-select form-select-sm">
                                        <!-- diisi via JS -->
                                    </select>
                                ...
                                <div class="col-6">
                                    ...
                                    <select name="po_number" id="po_number" class="form-select form-select-sm">
                                        ...
                                        <?php foreach ($poCustomerMap as $po => $info): ?>
                                            ...
                                            // Untuk label, ambil nama pertama saja biar pendek
...
<script>
// ========== Helper untuk memecah "(1) ... (2) ..." ==========
function parseIndexedList(raw) {
    raw = (raw || '').trim();
    if (!raw) return [];
    // samakan newline, lalu split di penanda (1)(2)(3)...
    raw = raw.replace(/\r\n/g, '\n');
...
// ============================
// PO SELECT -> Isi Nama & Alamat, plus dropdown kalau >1 customer
// ============================
...
function applyCustomerFromPo() {
    ...
    if (customers.length === 1) {
        // hanya satu → langsung isi, tidak perlu dropdown
        if (custChoiceWrapEl) custChoiceWrapEl.style.display = 'none';
    ...
    // lebih dari satu → tampilkan dropdown
