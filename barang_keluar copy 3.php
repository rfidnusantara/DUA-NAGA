...
// ==========================
// 1b. (Opsional) Daftar PO dari barang aktif (kalau masih mau dipakai di tempat lain)
// ==========================
$poStmt = $pdo->query("
...
// ==========================
// 1c. Ambil data API → Mapping PO -> (name, address)
// ==========================

// Bisa jadi fetch_sales_orders_from_api() mengembalikan:
// 1) array of orders langsung
// 2) atau ['data' => [orders...]]
$rawApi = fetch_sales_orders_from_api(1, 50);

// Normalisasi ke $salesOrdersApi = array of orders
if (is_array($rawApi) && isset($rawApi['data']) && is_array($rawApi['data'])) {
    $salesOrdersApi = $rawApi['data'];   // format JSON: { "data": [ ... ], ... }
} else {
    $salesOrdersApi = $rawApi;           // sudah berupa array of orders
}

// ['PO_NUMBER' => ['name' => '...', 'address' => '...']]
$poCustomerMap = [];

if (is_array($salesOrdersApi)) {
    foreach ($salesOrdersApi as $order) {
        // PO nomor dari API → customer_order.number
        $po = trim($order['customer']['customer_order']['number'] ?? $order['customer_order']['number'] ?? '');
        if ($po === '') {
            continue;
        }

        // Nama Customer langsung dari field "name" di objek customer
        $custName = trim($order['customer']['name'] ?? '');

        // Alamat lengkap langsung dari field "address" di objek customer
        $custAddress = trim($order['customer']['address'] ?? '');
...
// ==========================
// 3. Log Barang Keluar (HANYA 10 TERAKHIR)
// ==========================
$logStmt = $pdo->query("
                        ...
                        <div class="card bg-light border-0 p-3">
                            ...
                            <div class="row g-2">
                                <div class="col-6">
                                    ...
                                    <!-- GANTI: sekarang pakai SELECT dari API, lengkap dengan nama & alamat -->
                                    <select name="po_number" id="po_number" class="form-select form-select-sm">
                                        ...
                                        <?php foreach ($poCustomerMap as $po => $info): ?>
                                            <option
                                                ...
                                                data-address="<?= htmlspecialchars($info['address']); ?>"
                                                <?= (isset($_POST['po_number']) && $_POST['po_number'] === $po) ? 'selected' : ''; ?>
                                            >
                                                <?= htmlspecialchars($po . ' - ' . $info['name']); ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                    <div class="form-text small">
                                        PO, nama & alamat diambil langsung dari API.
                                    </div>
                                </div>
                                <div class="col-6">
...
<script>
// ============================
// PO SELECT -> Isi Nama & Alamat dari data-* option
// ============================
...
function applyCustomerFromPo() {
    ...
    custNameEl.value    = opt.dataset.name    || '';
    custAddressEl.value = opt.dataset.address || '';
}

if (poSelect) {
    poSelect.addEventListener('change', applyCustomerFromPo);
    // Jalankan sekali saat halaman selesai load (supaya POST sebelumnya tetap terisi)
    window.addEventListener('DOMContentLoaded', applyCustomerFromPo);
}

// ============================
// Scanner & Preview (tidak diubah)
// ============================
