...
# KONFIG READER HF340
# =======================
# IP HF340 (sesuaikan dengan jaringanmu)
RFID_IP = "172.16.36.4"   # ganti kalau perlu
RFID_PORTTCP = 8282       # port Hopeland HF340

# Perintah inventory per antena (ISI DENGAN HEX RESMI DARI HOPELAND)
# Default diisi sama untuk semua antena, kalau protokolnya memang sama.
# Kalau punya hex spesifik ANT2/3/4, ganti di sini.
...
# Hanya untuk info (berapa antena terpasang)
ANT_COUNT = 4
...
MAX_POLL_LOOPS   = 4
SEND_ROUNDS      = 1            # 1 command per loop (nanti di-rotate antar antena)
BATCH_FLUSH_MS   = 250
...
PATH_CTRL      = os.path.join(PARENT_FOLDER, "rfid_control.json")  # <-- file kontrol Start/Stop

PATH_RESULT    = os.path.join(PARENT_FOLDER3, "rfid.txt")          # EPC terakhir untuk di-GET PHP

# Status per antena (keterangan di log)
ANT_STATUS = {
    1: "Masuk",
    2: "Masuk",
    3: "Masuk",
...
# =======================
# Async Logger (NO DATABASE) - versi REVISI
# =======================
class AsyncLogger(threading.Thread):
    def __init__(self, queue, whitelist, stats):
        ...
        # flag: logger sedang aktif / nonaktif (dipakai saat START/STOP)
        self.log_active = True

    def stop(self):
        self.stop_event.set()

    def reset_jsonl(self):
        """
        Dipanggil saat STOP:
        - kosongkan read_log.jsonl
        - kosongkan cache latest (reads_latest.json)
        - CSV dibiarkan (supaya riwayat panjang tetap tersimpan)
        """
        try:
            # kosongkan file JSONL
            self.jsonl_f.seek(0)
            self.jsonl_f.truncate()
            self.jsonl_f.flush()
            print("[LOGGER] read_log.jsonl di-reset.")
        except Exception as e:
            print("[LOGGER] Gagal reset read_log.jsonl:", e)
        ...
        try:
            ...
            print("[LOGGER] Gagal reset reads_latest.json:", e)

    def flush(self):
        if not self.batch:
            return

        # Kalau STOP (log_active False) → jangan tulis apa pun
        ...
        for ev in self.batch:
            ...
            ant  = ev["ant"]
            rssi = ev["rssi"]
            code = epc           # untuk registrasi: code = EPC
            ...
            # Print ke console + tulis EPC terakhir ke PATH_RESULT
            if PRINT_EACH_EVENT:
                print(f"[READ] ANT{ant} EPC={epc} RSSI={rssi} {ket}")
            try:
                # hanya EPC terakhir, bisa dibaca PHP
                with open(PATH_RESULT, "w", encoding="utf-8") as f:
                    f.write(epc)
            except Exception as e:
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            # Jika logger nonaktif (STOP) → buang queue & batch
            if not self.log_active:
                while self.q:
                    try:
                        self.q.popleft()
        ...
        # stop → flush sisa (kalau masih aktif)
        self.flush()
...
def main():
    ...
    try:
        while True:
            # ====== BACA FILE KONTROL START/STOP ======
            ctrl = load_json(PATH_CTRL, {"enabled": False})
            enabled = bool(ctrl.get("enabled", False))

            if enabled != last_enabled:
                # status berubah -> tampilkan info
                print(f"[CTRL] enabled = {enabled}")

                if not enabled:
                    # Transisi: ON -> OFF (STOP)
                    # 1) Nonaktifkan logger agar tidak nulis
                    logger.log_active = False
                    # 2) Reset file JSONL & snapshot latest
                    logger.reset_jsonl()
                else:
                    # Transisi: OFF -> ON (START)
                    # Aktifkan kembali logger
                    logger.log_active = True

                last_enabled = enabled

            if not enabled:
                # status STOP -> tutup koneksi reader & idle
                if rc.sock:
                    try:
                        rc.close()
            ...
            if not rc.sock:
                # kalau belum connect tapi status enabled = True -> konek
                try:
                    rc.connect()
